...
import { InsertAsetTetap } from "../api/aset-tetap";
...
export default class Insert extends Component {
  ...
  saveData = async () => {
    ...
    const nominal = $("#insert-aset-tetap-form .nominal")
      .val()
      .replace(/\,/gi, "");
    const keterangan = await upperCase(
      $("#insert-aset-tetap-form .keterangan").val()
    );
    ...
    if (nominal != "" && keterangan != "") {
      ...
      if (cekModal.status == false) {
        ...
        Notif.send({ title: "Input Aset Tetap", body: modalMsg });
  ...
  errorHighlight = () => {
    ...
    const keteranganInput = $(
      "#insert-aset-tetap-form .form-group .keterangan"
    );
    ...
    // Nominal
    const nominalInput = $("#insert-aset-tetap-form .form-group .nominal");
  ...
  reformatNumber = async (getVal, target) => {
    ...
    $("#insert-aset-tetap-form ." + target).val(newVal);
  };

  removeForm = () => {
    $("#insert-aset-tetap-form")[0].reset();
    this.props.close();
  };
  ...
  render() {
    return (
      ...
      >
        <form id="insert-aset-tetap-form" onSubmit={(e) => e.preventDefault()}>
          <h1
