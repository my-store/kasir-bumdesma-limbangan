import { UpdatePersediaan, GetOneObat } from "../../api/persediaan";
...
export default class Update extends Component {
  ...
  updateData = async () => {
    ...
    const id = $("#update-obatPertanian-form .id").val();
    const namaBarang = await upperCase(
      $("#update-obatPertanian-form .nama-barang").val()
    );
    const satuan = await upperCase(
      $("#update-obatPertanian-form .satuan").val()
    );

    // Hapus semua koma
    const stok = $("#update-obatPertanian-form .stok")
      .val()
      .replace(/\,/gi, "");
    const hargaPokok = $("#update-obatPertanian-form .hargaPokok")
      .val()
      .replace(/\,/gi, "");
    const hargaJual = $("#update-obatPertanian-form .hargaJual")
      .val()
      .replace(/\,/gi, "");
    ...
    if (
      ...
      if (
        ...
        const getTimestamp = $("#update-obatPertanian-form .timestamp");
      ...
      const oldData = await GetOneObat({ id });
      ...
      if (data.hargaPokok != oldData.hargaPokok || data.stok != oldData.stok) {
        ...
        if (cekModal.status == false) {
          ...
          Notif.send({ title: "Ubah Obat Pertanian", body: modalMsg });
      ...
      // Update data
      await UpdatePersediaan({ id: "Obat", data });
  ...
  reformatNumber = async (getVal, target) => {
    ...
    $("#update-obatPertanian-form ." + target).val(newVal);
  };

  listenTimestamp = () => {
    const tanggal = $(
      "#update-obatPertanian-form .timestamp-input input[placeholder='Tanggal']"
    );
    const bulan = $(
      "#update-obatPertanian-form .timestamp-input input[placeholder='Bulan']"
    );
    const tahun = $(
      "#update-obatPertanian-form .timestamp-input input[placeholder='Tahun']"
    );
  ...
  removeForm = () => {
    $("#update-obatPertanian-form")[0].reset();
  ...
  removeTimestamp = () => {
    ...
    const ti = $("#update-obatPertanian-form .timestamp-input input");
    for (let tx of ti) {
      $(tx).val(null);
    }
  ...
  render() {
    ...
    return (
      <form id="update-obatPertanian-form">
        {/* Hidden data */}
        <input
          type="hidden"
          className="id"
