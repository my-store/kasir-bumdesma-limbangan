import { UpdatePersediaan, GetOnePupuk } from "../../api/persediaan";
...
export default class Update extends Component {
  ...
  updateData = async () => {
    ...
    const id = $("#update-pupuk-form .id").val();
    const namaBarang = await upperCase(
      $("#update-pupuk-form .nama-barang").val()
    );
    // Stok
    const stok = $("#update-pupuk-form .stok").val().replace(/\,/gi, "");
    // Satuan
    const satuan = await upperCase($("#update-pupuk-form .satuan").val());
    // Harga pokok
    const hargaPokok = $("#update-pupuk-form .hargaPokok")
      .val()
      .replace(/\,/gi, "");
    // Harga jual
    const hargaJual = $("#update-pupuk-form .hargaJual")
      .val()
      .replace(/\,/gi, "");
    ...
    if (satuan != "" && hargaPokok != "" && stok != "" && hargaJual != "") {
      ...
      if (
        ...
        const getTimestamp = $("#update-pupuk-form .timestamp");
      ...
      const oldData = await GetOnePupuk({ id });
      ...
      if (data.hargaPokok != oldData.hargaPokok || data.stok != oldData.stok) {
        ...
        if (cekModal.status == false) {
          ...
          Notif.send({ title: "Ubah Pupuk Nonsubsidi", body: modalMsg });
      ...
      // Update data
      await UpdatePersediaan({ id: "Pupuk", data });
  ...
  reformatNumber = async (getVal, target) => {
    ...
    $("#update-pupuk-form ." + target).val(newVal);
  };

  listenTimestamp = () => {
    const tanggal = $(
      "#update-pupuk-form .timestamp-input input[placeholder='Tanggal']"
    );
    const bulan = $(
      "#update-pupuk-form .timestamp-input input[placeholder='Bulan']"
    );
    const tahun = $(
      "#update-pupuk-form .timestamp-input input[placeholder='Tahun']"
    );
  ...
  removeTimestamp = () => {
    ...
    const ti = $("#update-pupuk-form .timestamp-input input");
    for (let tx of ti) {
      $(tx).val(null);
    }
