import { UpdateOperasional, GetOneOperasional } from "../api/operasional";
...
export default class Update extends Component {
  ...
  updateData = async () => {
    ...
    // ID
    const id = $("#update-operasional-form .id").val();
    // Keterangan
    const keterangan = await upperCase(
      $("#update-operasional-form .keterangan").val()
    );
    // Jumlah
    const jumlah = $("#update-operasional-form .jumlah")
      .val()
      .replace(/\,/gi, "");

    // Tipe biaya (gaji, listrik dll)
    const tipe = $("#update-operasional-form .tipe-operasional").val();
    ...
    if (keterangan != "" && jumlah != "") {
      ...
      if (
        ...
        const getTimestamp = $("#update-operasional-form .timestamp");
      ...
      const oldData = await GetOneOperasional({ id });

      // Update 2025
      // Tidak ada perubahan data
      const nothingChange =
        data.keterangan == oldData.keterangan &&
        data.jumlah == oldData.jumlah &&
        data.tipe == oldData.tipe &&
      ...
      // Update 2025
      // Jika jumlah berubah, cek kembali sisa uang kas
      if (data.jumlah != oldData.jumlah) {
        // Cek modal/ kas
        ...
        if (cekModal.status == false) {
          ...
          Notif.send({ title: "Ubah Operasional", body: modalMsg });
  ...
  errorHighlight = () => {
    ...
    const keteranganInput = $(
      "#update-operasional-form .form-group .keterangan"
    );
    if (keteranganInput.val().length < 1)
      keteranganInput.css("border", "1px solid red");
    // Jumlah
    const jumlahInput = $("#update-operasional-form .form-group .jumlah");
  ...
  reformatNumber = async (getVal, target) => {
    ...
    $("#update-operasional-form ." + target).val(newVal);
  };

  listenTimestamp = () => {
    const tanggal = $(
      "#update-operasional-form .timestamp-input input[placeholder='Tanggal']"
    );
    const bulan = $(
      "#update-operasional-form .timestamp-input input[placeholder='Bulan']"
    );
    const tahun = $(
      "#update-operasional-form .timestamp-input input[placeholder='Tahun']"
    );
  ...
  getTipeOperasional = (getData) => {
    ...
    return (
      <div className="operasional-list">
        {tipe.map((data, index) => {
          const checked = data.text == getData.tipe ? true : false;
          return (
            <div
              key={index}
              className="operasional-item"
              onClick={() => {
                $("#update-operasional-form ." + tipe[index].class).prop(
                  "checked",
                  true
                );
                $("#update-operasional-form .tipe-operasional").val(
                  tipe[index].text
                );
              }}
            >
  ...
  removeForm = () => {
    $("#update-operasional-form")[0].reset();
  ...
  removeTimestamp = () => {
    ...
    const ti = $("#update-operasional-form .timestamp-input input");
    for (let tx of ti) {
      $(tx).val(null);
    }
  ...
  render() {
    ...
    return (
      ...
      >
        <form id="update-operasional-form" onSubmit={(e) => e.preventDefault()}>
          <input
            type="hidden"
            className="id"
          ...
          <input
            type="hidden"
            className="tipe-operasional"
            defaultValue={data != null ? data.tipe : null}
          />
          ...
          <div className="form-group">
            ...
            <input
              type="text"
              className="jumlah"
              defaultValue={data != null ? numberFormat(data.jumlah) : null}
          ...
          <div className="form-group">
            ...
            {data != null ? this.getTipeOperasional(data) : null}
          </div>
